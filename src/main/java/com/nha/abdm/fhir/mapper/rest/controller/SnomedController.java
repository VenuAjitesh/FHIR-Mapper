...
@Tag(name = SwaggerConstants.SNOMED_CONTROLLER_TAG, description = SwaggerConstants.SNOMED_CONTROLLER_DESCRIPTION)
public class SnomedController {
  @Autowired SnomedService snomedService;

  @GetMapping({"/{resource}"})
  @Operation(summary = SwaggerConstants.GET_SNOMED_CODES_SUMMARY, description = SwaggerConstants.GET_SNOMED_CODES_DESCRIPTION)
  @ApiResponses(value = {
          @ApiResponse(responseCode = SwaggerConstants.HTTP_200, description = SwaggerConstants.SNOMED_SUCCESS_DESCRIPTION),
          @ApiResponse(responseCode = SwaggerConstants.HTTP_400, description = SnomedCodeIdentifier.INVALID_RESOURCE)
  })
  public ResponseEntity<SnomedResponse> getSnomedCodes(
          @Parameter(description = SwaggerConstants.SNOMED_RESOURCE_PARAMETER_DESCRIPTION) @PathVariable("resource") String resource) {
